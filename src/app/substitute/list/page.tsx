...
export default function SubstituteListPage() {
  const PAGE_SIZE = 10;
  ...
  const loadRows = async () => {
    ...
    try {
      ...
      usersSnap.forEach((d) => {
        ...
        nextUserNameById[d.id] = typeof data.name === "string" && data.name.trim() ? data.name.trim() : d.id;
      });
  ...
  const rangeStart =
    filteredRows.length === 0 ? 0 : (effectivePage - 1) * PAGE_SIZE + 1;
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full rounded-md border border-gray-300 px-3 py-2 text-base focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100"
                />
              </div>
